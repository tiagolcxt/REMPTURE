...
const CHARACTER_START_POSITION := Vector2i(320, 486)
...
const GHOST_START_POSITION := Vector2i(320, 486)  # Posição inicial do fantasma
const GHOST_X_OFFSET := -300  # Distância padrão atrás da câmera
const GHOST_Y_OFFSET := 0.0  # Altura base em relação ao chão

const GHOST_WAVE_AMPLITUDE := 20.0  # Altura da onda de flutuação
const GHOST_WAVE_FREQUENCY := 2.0  # Velocidade da onda
const GHOST_FOLLOW_SPEED := 10.0  # Suavização do movimento
